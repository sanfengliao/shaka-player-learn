...
export class InitSegmentReference {
  ...
  /**
   * @param {function():string[]} uris A function that creates the URIs
   *   of the resource containing the init
  /**
   ...
   */
  static equal(
    reference1: InitSegmentReference,
    reference2: InitSegmentReference
  ) {
    if (reference1 === reference2) {
      ...
      return (
        ...
        ArrayUtils.equal(
          reference1.getUris().sort(),
          reference2.getUris().sort()
        ) &&
        BufferUtils.equal(
          reference1.getSegmentData(),
...
export class SegmentReference {
   ...
   * @param {number} endTime The segment's end time
  ...
  constructor(
    ...
    status = SegmentReference.Status.AVAILABLE,
    aesKey: AesKey | null = null,
 ...
 */
export type AnySegmentReference = InitSegmentReference | SegmentReference;
