...
import { CmsdConfiguration } from '../../externs/shaka';
 ...
 * @export
 */
export class CmsdManager {
  private static CMSD_STATIC_HEADER_NAME_ = 'cmsd-static';

  /**
   * @const {string}
   * @private
   */
  private static CMSD_DYNAMIC_HEADER_NAME_ = 'cmsd-dynamic';
  ...
  config_: CmsdConfiguration;
  staticParams_: Map<string, boolean | number | string> | null;
  dynamicParams_: Map<string, boolean | number | string> | null;

  constructor(config: CmsdConfiguration) {
    this.config_ = config;
   ...
   *
   */
  configure(config: CmsdConfiguration) {
    this.config_ = config;
  }
   ...
   *
   */
  processHeaders(headers: Record<string, string>) {
   ...
   */
  getMaxBitrate() {
    const key = CmsdManager.KEYS_.MAX_SUGGESTED_BITRATE;
    if (
      !this.config_.enabled ||
      !this.config_.applyMaximumSuggestedBitrate ||
      !this.dynamicParams_ ||
      !this.dynamicParams_.has(key)
    ) {
      return null;
    }
    return Number(this.dynamicParams_.get(key)) * 1000;
  }
  ...
  getEstimatedThroughput() {
    const key = CmsdManager.KEYS_.ESTIMATED_THROUGHPUT;
    if (
      !this.config_.enabled ||
      !this.dynamicParams_ ||
      !this.dynamicParams_.has(key)
   ...
   */
  getResponseDelay() {
    const key = CmsdManager.KEYS_.RESPONSE_DELAY;
    if (
      !this.config_.enabled ||
      !this.dynamicParams_ ||
      !this.dynamicParams_.has(key)
    ...
    return Number(this.dynamicParams_.get(key));
  }
   ...
   */
  getRoundTripTime() {
    const key = CmsdManager.KEYS_.ROUND_TRIP_TIME;
    if (
      !this.config_.enabled ||
      !this.dynamicParams_ ||
      !this.dynamicParams_.has(key)
  ...
  getBandwidthEstimate(defaultEstimate: number) {
    ...
    if (etpWeightRatio > 0 && etpWeightRatio <= 1) {
      return (
        defaultEstimate * (1 - etpWeightRatio) +
  ...
  private parseCMSDStatic_(
    headerValue: string
  ): Map<string, boolean | number | string> | null {
    try {
      ...
      log.warning('Failed to parse CMSD-Static response header value:', e);
      return null;
    }
  }

  private parseCMSDDynamic_(
    headerValue: string
  ): Map<string, boolean | number | string> | null {
    try {
      const params = new Map();
      ...
      log.warning('Failed to parse CMSD-Dynamic response header value:', e);
      return null;
    }
  }

  private parseParameterValue_(value: string): boolean | number | string {
