...
export class EbmlParser {
  ...
  constructor(data: BufferSource) {
    ...
    this.reader_ = new DataViewReader(this.dataView_, DataViewReaderEndianness.BIG_ENDIAN);
  }
  ...
  parseId_() {
    ...
    if (vint.length > 7) {
      throw new ShakaError(ShakaError.Severity.CRITICAL, ShakaError.Category.MEDIA, ShakaError.Code.EBML_OVERFLOW);
    }
   ...
   * @private
   */
  static getVintValue_(vint: Uint8Array) {
   ...
   * @private
   */
  static isDynamicSizeValue_(vint: Uint8Array) {
...
export class EbmlElement {
   ...
   */
  constructor(id: number, dataView: DataView) {
    this.id = id;
