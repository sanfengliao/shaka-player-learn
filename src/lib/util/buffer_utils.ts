...
export class BufferUtils {
   ...
   * @private
   */
  static unsafeGetArrayBuffer_(view: BufferSource): ArrayBuffer {
   ...
   * @export
   */
  static toUint8(data: BufferSource, offset = 0, length = Infinity) {
   ...
   * @export
   */
  static toUint16(data: BufferSource, offset = 0, length = Infinity) {
    return BufferUtils.view_(data, offset, length, Uint16Array);
  }
   ...
   * @export
   */
  static toDataView(buffer: BufferSource, offset = 0, length = Infinity) {
    // @ts-ignore
  ...
  static view_<T>(
    ...
    offset: number,
    length: number,
    Type: new (data: BufferSource, offset: number, length: number) => T
  ): T {
