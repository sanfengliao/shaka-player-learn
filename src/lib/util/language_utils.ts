...
export class LanguageUtils {
   ...
   * @export
   */
  static areLocaleCompatible(locale1: string, locale2: string) {
   ...
   * @export
   */
  static areLanguageCompatible(locale1: string, locale2: string) {
    // Even through they SHOULD already be normalized, let's just be safe and
    // do it again.
    locale1 = LanguageUtils.normalize(locale1);
    locale2 = LanguageUtils.normalize(locale2);
   ...
   * @export
   */
  static isParentOf(possibleParent: string, possibleChild: string) {
    // Even through they SHOULD already be normalized, let's just be safe and
    // do it again.
    ...
    return (
      possibleParentComponents[0] == possibleChildComponents[0] &&
   ...
   * @export
   */
  static isSiblingOf(localeA: string, localeB: string) {
    // Even through they SHOULD already be normalized, let's just be safe and
    // do it again.
  ...
  static normalize(locale: string) {
    ...
    const [languageRegion = '', privateuseSuffix = ''] = locale.split(
      `-${privateusePrefix}`
    );
   ...
   * @export
   */
  static areSiblings(a: string, b: string) {
   ...
   * @export
   */
  static relatedness(target: string, candidate: string) {
   ...
   * @export
   */
  static getLocaleForText(stream: Stream) {
   ...
   * @export
   */
  static getLocaleForVariant(variant: Variant) {
  ...
  static findClosestLocale(target: string, searchSpace: Iterable<string>) {
    ...
    /** @type {!Set.<string>} */
    const safeSearchSpace = new Set<string>();
   ...
   * @private
   */
  static disassembleLocale_(locale: string) {
