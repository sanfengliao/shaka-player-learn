 ...
 * @export
 */
export class MediaSourcePolyfill {
  /**
  ...
  static install() {
    ...
    if (!window.MediaSource && !window.ManagedMediaSource) {
      ...
      if (safariVersion <= 10) {
        ...
        log.info('Blacklisting MSE on Safari <= 10.');
        MediaSourcePolyfill.blacklist_();
        ...
        // Bug filed: https://bugs.webkit.org/show_bug.cgi?id=165342
        MediaSourcePolyfill.stubAbort_();
        ...
        MediaSourcePolyfill.patchRemovalRange_();
      ...
      MediaSourcePolyfill.rejectCodec_('opus');
    } else {
    ...
    if (window.MediaSource || window.ManagedMediaSource) {
      ...
      MediaSourcePolyfill.rejectContainer_('mp2t');
    }
    ...
    if (
      ...
      MediaSourcePolyfill.patchVp09_();
    }
  }
   ...
   * @private
   */
  static rejectContainer_(container: string) {
    const isTypeSupported =
      // eslint-disable-next-line no-restricted-syntax
      MediaSource.isTypeSupported.bind(MediaSource);

    MediaSource.isTypeSupported = (mimeType) => {
      const actualContainer = MimeUtils.getContainerType(mimeType);
    ...
    if (window.ManagedMediaSource) {
      ...
      // @ts-expect-error
      window.ManagedMediaSource.isTypeSupported = (mimeType: string) => {
        const actualContainer = MimeUtils.getContainerType(mimeType);
   ...
   * @private
   */
  static rejectCodec_(codec: string) {
    const isTypeSupported =
      // eslint-disable-next-line no-restricted-syntax
      MediaSource.isTypeSupported.bind(MediaSource);
...
polyfill.register(MediaSourcePolyfill.install);
